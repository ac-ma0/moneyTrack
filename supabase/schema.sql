...
create table if not exists public.app_settings (user_id uuid primary key references auth.users(id) on delete cascade, dark_theme boolean default false, currency text default 'PHP', theme_settings jsonb not null default '{}'::jsonb, updated_at timestamptz default now());
alter table public.app_settings add column if not exists theme_settings jsonb not null default '{}'::jsonb;
...
create policy "profiles own rows" on public.profiles for all using (auth.uid() = id or public.has_permission('view_all_records')) with check (auth.uid() = id or public.is_admin());
