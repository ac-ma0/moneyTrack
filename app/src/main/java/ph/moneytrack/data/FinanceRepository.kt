...
class FinanceRepository(private val db: FinanceDatabase, private val userId: String) {
    ...
    private suspend fun write(operation:String,type:String,id:String,newValue:Any?, action:suspend()->Unit) {
        db.withTransaction {
            ...
            action()
            val payload = newValue?.toJson() ?: JSONObject().apply {
                put("id", id)
                put("user_id", userId)
                put("deleted", true)
                put("updated_at", System.currentTimeMillis())
            }.toString()
            db.audit().insert(AuditLog(userId=userId,action=operation,recordType=type,recordId=id,oldValue=old,newValue=payload))
    ...
    private fun Any.toJson(): String = JSONObject().apply {
        when (this@toJson) {
            is Income -> { put("id",id); put("user_id",userId); put("title",title); put("amount",amount/100.0); put("occurred_on",occurredOn); put("category",category); put("notes",notes); put("updated_at",updatedAt); put("deleted",deleted) }
            is Expense -> { put("id",id); put("user_id",userId); put("title",title); put("amount",amount/100.0); put("occurred_on",occurredOn); put("category",category); put("notes",notes); put("updated_at",updatedAt); put("deleted",deleted) }
            is Debt -> { put("id",id); put("user_id",userId); put("person",person); put("principal_amount",principalAmount/100.0); put("interest_rate",interestRate); put("interest_type",interestType); put("number_of_months",numberOfMonths); put("start_date",startDate); put("due_date",dueDate); put("monthly_expected_payment",monthlyExpectedPayment?.div(100.0)); put("total_payable",totalPayable?.div(100.0)); put("notes",notes); put("status",status); put("updated_at",updatedAt); put("deleted",deleted) }
            is DebtMonthlyPayment -> { put("id",id); put("debt_id",debtId); put("user_id",userId); put("payment_month",paymentMonth); put("amount",amount/100.0); put("status",status); put("notes",notes); put("updated_at",updatedAt); put("deleted",deleted) }
